...
const Dashboard = () => {
    ...
    const maxTimeSpent = selectedProject ? Math.max(...stats.projectsWithWorkedTime.find(project => project._id === selectedProject).tasks.map(task => task.timeSpent)) : 0;
    const maxEstimatedTime = selectedProject ? Math.max(...stats.projectsWithWorkedTime.find(project => project._id === selectedProject).tasks.map(task => task.estimatedTime)) : 0;
    const maxYAxis = Math.max(maxTimeSpent, maxEstimatedTime) || 1000;
    const selectedProjectData = selectedProject ? stats.projectsWithWorkedTime.find(project => project._id === selectedProject) : null;
    ...
    var optionsBarVerticalTasksTime = {
        ...
        xaxis: {
            categories: selectedProject ? stats.projectsWithWorkedTime.find(project => project._id === selectedProject).tasks.map(task => task.nameTask) : []
        },
        yaxis: {
            show: true,
            min: 0,
        ...
        series: selectedProject ? [
            { name: 'Temps passé (heures)', data: selectedProjectData.tasks.map(task => task.timeSpent) },
            { name: 'Temps estimé (heures)', data: selectedProjectData.tasks.map(task => task.estimatedTime) }
        ] : []
    }
    var optionsBarHorizontalProjectsPerTeam = {
        ...
        yaxis: {
            show: true,
            min: 0,
            max: Math.max(...stats.teamsAndTheirProjects.map(team => team.projects.length)) || 1000, // You can adjust the default maximum value as needed
            tickAmount: 4,
            labels: {
                style: {
                    cssClass: "grey--text lighten-2--text fill-color",
    ...
    var optionsBarHorizontalEmployeesPerTeam = {
        ...
        yaxis: {
            show: true,
            min: 0,
            max: Math.max(...stats.teamsAndTheirProjects.map(team => team.employees.length)) || 1000, // You can adjust the default maximum value as needed
            tickAmount: 4,
            labels: {
                style: {
                    cssClass: "grey--text lighten-2--text fill-color",
    ...
    var donutOptionsProjects = {
        ...
        ,
        series: stats ? [stats.projectsNotStarted,
        stats.projectsInProgress,
        stats.projectsFinished] : [0, 0, 0]
    }
    ...
    var donutOptionsTasks = {
        ...
        labels: ['En attente', 'En cours', 'Terminé']
        ,
        series: stats ? [stats.tasksNotStarted,
        stats.tasksInProgress,
        stats.tasksFinished] : [0, 0, 0]
    }
    ...
    return (
        <div className="container-fluid">
            {localStorage.getItem('role') === 'employe' ?
                <div className='d-flex justi-content-center align-items-center' style={{ height: '80vh' }}><h1>Vous n'êtes pas autorisé à voir cette page</h1> </div> :
                <div className="row">
                            ...
                            <div className="card-body">
                                <div className="d-sm-flex d-block align-items-center justify-content-between mb-9">
                                    <div className="mb-3 mb-sm-0">
                                        <h5 className="card-title fw-semibold">Statistiques verticales</h5>
                                    </div>
                                </div>
                                {stats && <Chart
                    ...
                    <div className="col-lg-6">
                        <div className="card overflow-hidden">
                            <div className="card-body p-4">
                                <h5 className="card-title mb-9 fw-semibold">Nombre de projets par équipe</h5>
                                <div className="row align-items-center">
                                    {stats && <Chart
                    ...
                    <div className="col-lg-6">
                        <div className="card overflow-hidden">
                            <div className="card-body p-4">
                                <h5 className="card-title mb-9 fw-semibold">Nombre d'employés par équipe</h5>
                                <div className="row align-items-center">
                                    {stats && <Chart
